...
export default async function adminListingsRoutes(fastify: FastifyInstance) {
  ...
  // GET /admin/listings/:id/containers - Get containers for a listing
  fastify.get('/:id/containers', { preHandler: [adminAuth] }, async (request, reply) => {
    try {
      const { id } = request.params as any;
      console.log('📦 Admin GET listing containers - Listing ID:', id);

      // ✅ Admin thấy TẤT CẢ containers (không filter theo status)
      ...
      // ✅ Thêm summary để admin theo dõi tổng quan
      ...
      console.log(`✅ Found ${containers.length} containers for listing ${id}`);
      ...
      return reply.send({
        ...
        data: {
          containers: containers.map(c => ({
            ...
            status: c.status,  // ✅ Include status for admin view
      ...
      console.error('❌ Admin get listing containers error:', error);
      return reply.status(500).send({
        success: false,
        message: 'Internal server error',
        error: (error as Error).message
