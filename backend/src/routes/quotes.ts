...
import { getRFQReservationService, RFQReservationService } from '../lib/rfq/rfq-reservation-service.js';
...
export default async function quoteRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { id: string } }>('/:id/test-accept', async (request, reply) => {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const selectedContainerIds = quote.rfqs.selected_container_ids as string[] || [];
        const effectiveQuantity = selectedContainerIds.length > 0 
          ? selectedContainerIds.length 
          : quote.quote_items.reduce((sum, item) => sum + Number(item.qty), 0);

        console.log('🧪 [TEST] Container IDs from RFQ:', selectedContainerIds);
        console.log('🧪 [TEST] Effective Quantity:', effectiveQuantity);
        ...
        // 🆕 Reserve inventory and link containers to order
        if (selectedContainerIds.length > 0) {
          ...
          // Get deal_type from listing or default to SALE
          const dealType = quote.rfqs.listing?.deal_type || 'SALE';
          ...
          console.log(`🧪 [TEST] Reserved ${effectiveQuantity} containers for order ${order.id}, Deal Type: ${dealType}, Duration: ${rentalDurationMonths} months`);
          console.log(`   Container IDs: ${selectedContainerIds.join(', ')}`);
        } else {
          // No specific containers selected, just decrement available_quantity
          const { InventoryService } = await import('../lib/inventory/inventory-service');
          const inventoryService = new InventoryService(tx as any);
          ...
          console.log(`🧪 [TEST] Reserved ${effectiveQuantity} units for order ${order.id} (no specific containers), Deal Type: ${dealType}`);
        }
  ...
  fastify.post<{ Params: { id: string } }>('/:id/accept', {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        console.log('🔍 [Accept Quote] Container IDs from RFQ:', selectedContainerIds);
        console.log('🔍 [Accept Quote] Effective Quantity:', effectiveQuantity);
        ...
        const order = await tx.orders.create({
          data: {
            ...
            total,
            currency: quote.currency,
            deal_type: orderDealType, // ✅ Set deal_type từ RFQ
            rental_duration_months: orderDealType === 'RENTAL' ? rfqRentalDuration : null, // ✅ Set rental duration
            updated_at: new Date(),
            order_items: {
              create: quote.quote_items.map(item => ({
                id: uuidv4(),
        ...
        // 🆕 Reserve inventory and link containers to order
        if (selectedContainerIds.length > 0) {
          // 🔄 [RFQ RESERVATION] Convert RESERVED → SOLD/RENTED based on deal_type
          const dealType = order.deal_type || 'SALE';
          const rentalDurationMonths = order.rental_duration_months || null;
          console.log(`🔄 [Accept Quote] Converting RESERVED containers for RFQ ${quote.rfq_id}, Deal Type: ${dealType}, Duration: ${rentalDurationMonths} months`);
          
          try {
            const { RFQReservationService } = await import('../lib/rfq/rfq-reservation-service');
            console.log('✅ [Accept Quote] RFQReservationService imported successfully');
            
            // Pass prisma as constructor param, then pass tx as transactionClient
            const rfqService = new RFQReservationService(prisma);
            console.log('✅ [Accept Quote] RFQReservationService instantiated');
            
            const result = await rfqService.convertReservationToSold(
              quote.rfq_id, 
              order.id, 
              dealType, 
            ...
            console.log(`✅ [Accept Quote] Converted ${result} containers from RESERVED for order ${order.id}`);
            console.log(`   Container IDs from RFQ: ${selectedContainerIds.join(', ')}`);
          } catch (reservationError: any) {
            console.error('❌ [Accept Quote] Error in RFQ reservation:', reservationError);
            throw new Error(`Failed to convert reservations: ${reservationError.message}`);
          }
        } else {
          // No specific containers selected, just decrement available_quantity
          const { InventoryService } = await import('../lib/inventory/inventory-service');
          ...
          console.log(`✅ [Accept Quote] Reserved ${effectiveQuantity} units for order ${order.id} (no specific containers), Deal Type: ${dealType}`);
        }
  ...
  const handleQuoteRejection = async (request: any, reply: any) => {
    ...
    try {
      ...
      // 🔓 [RFQ RESERVATION] Check if this is the last active quote
      const activeQuotesCount = await prisma.quotes.count({
        where: {
          rfq_id: quote.rfq_id,
          status: { in: ['SENT', 'PENDING', 'sent', 'pending'] }
        }
      });

      console.log(`📊 [RFQ Reservation] RFQ ${quote.rfq_id} has ${activeQuotesCount} active quotes`);
      ...
      // 🔓 [RFQ RESERVATION] If this was the last active quote, release containers
      if (activeQuotesCount === 1) {
        console.log(`🔓 [RFQ Reservation] Last active quote rejected for RFQ ${quote.rfq_id}. Releasing containers...`);
        
        const rfqService = getRFQReservationService(prisma);
        const releasedCount = await rfqService.releaseRFQReservation(quote.rfq_id);
        
        console.log(`✅ [RFQ Reservation] Released ${releasedCount} containers back to AVAILABLE status`);
      } else {
        console.log(`ℹ️  [RFQ Reservation] RFQ ${quote.rfq_id} still has ${activeQuotesCount - 1} active quotes. Keeping containers reserved.`);
      }
