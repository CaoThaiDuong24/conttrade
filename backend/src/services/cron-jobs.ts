...
export function initializeCronJobs() {
  ...
  // 🆕 RENTAL MANAGEMENT CRON JOBS
  
  // Update overdue rental contracts: Chạy mỗi ngày lúc 1:00 AM
  cron.schedule('0 1 * * *', async () => {
    console.log('⏰ [CRON] Running update overdue rental contracts job at 1:00 AM');
    const { RentalContractService } = await import('./rental-contract-service.js');
    await RentalContractService.updateOverdueContracts();
  });

  // Calculate late fees: Chạy mỗi ngày lúc 2:00 AM
  cron.schedule('0 2 * * *', async () => {
    console.log('⏰ [CRON] Running calculate late fees job at 2:00 AM');
    const { RentalContractService } = await import('./rental-contract-service.js');
    await RentalContractService.calculateLateFees();
  });

  // Process auto-renewals: Chạy mỗi ngày lúc 3:00 AM
  cron.schedule('0 3 * * *', async () => {
    console.log('⏰ [CRON] Running process auto-renewals job at 3:00 AM');
    const { RentalContractService } = await import('./rental-contract-service.js');
    await RentalContractService.processAutoRenewals();
  });

  // Send rental payment reminders (3 days before): Chạy mỗi ngày lúc 9:00 AM
  cron.schedule('0 9 * * *', async () => {
    console.log('⏰ [CRON] Running send rental payment reminders job at 9:00 AM');
    const { RentalContractService } = await import('./rental-contract-service.js');
    await RentalContractService.sendPaymentReminders(3);
  });

  // Retry failed payments: Chạy mỗi 6 giờ
  cron.schedule('0 */6 * * *', async () => {
    console.log('⏰ [CRON] Running retry failed payments job (every 6 hours)');
    const { RentalContractService } = await import('./rental-contract-service.js');
  ...
  console.log('   📅 Update overdue rental contracts: Every day at 1:00 AM');
  console.log('   📅 Calculate late fees: Every day at 2:00 AM');
  console.log('   📅 Process auto-renewals: Every day at 3:00 AM');
  console.log('   📅 Send rental payment reminders: Every day at 9:00 AM');
