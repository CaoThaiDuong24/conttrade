 ...
 * 2. Master Data (45 bảng md_* - TẤT CẢ các bảng master data theo schema.prisma)
 * 
 * Run: node scripts/seed/seed-complete.mjs
 * 
 * Danh sách 45 bảng md_* được seed (theo schema.prisma):
 * - md_countries, md_currencies, md_provinces, md_cities
 * - md_container_sizes, md_container_types
 * - md_quality_standards, md_iso_container_codes, md_deal_types
 * - md_unlocodes, md_adjust_reasons, md_business_hours_policies
 * - md_cancel_reasons, md_commission_codes
 ...
 * - md_i18n_namespaces, md_inspection_item_codes
 * - md_insurance_coverages, md_integration_vendor_codes, md_incoterms
 * - md_listing_statuses, md_movement_types
 * - md_notification_channels, md_notification_event_types
 * - md_order_statuses, md_payment_failure_reasons
 * - md_payment_statuses, md_payment_method_types, md_partner_types
 * - md_pricing_regions, md_rating_scales
 * - md_redaction_channels, md_ref_doc_types, md_rental_units
 * - md_repair_item_codes, md_service_codes
...
async function seedRBAC() {
  ...
  const permissions = [
    ...
    // Finance Permissions (PM-090 to PM-091B)
...
// ============================================================================
// SECTION 2: MASTER DATA (45 tables)
// ============================================================================
...
async function seedMasterData() {
  ...
  // STEP 1: Seed countries first (required for provinces)
  ...
  for (const country of countries) {
    await prisma.md_countries.upsert({
      ...
      update: {},
      create: { ...country, updated_at: new Date() }
    });
  }
  console.log(`✅ md_countries: seeded ${countries.length} records`);

  // STEP 2: Seed provinces (requires country_id)
  console.log('📍 Seeding md_provinces...');
  const provinces = [
    { id: 'province-hcm', country_id: 'country-vn', code: 'HCM', name: 'TP. Hồ Chí Minh', name_en: 'Ho Chi Minh City' },
    { id: 'province-hn', country_id: 'country-vn', code: 'HN', name: 'Hà Nội', name_en: 'Hanoi' },
    { id: 'province-dn', country_id: 'country-vn', code: 'DN', name: 'Đà Nẵng', name_en: 'Da Nang' },
    { id: 'province-hp', country_id: 'country-vn', code: 'HP', name: 'Hải Phòng', name_en: 'Hai Phong' },
    { id: 'province-ct', country_id: 'country-vn', code: 'CT', name: 'Cần Thơ', name_en: 'Can Tho' },
    { id: 'province-bd', country_id: 'country-vn', code: 'BD', name: 'Bình Dương', name_en: 'Binh Duong' },
    { id: 'province-dn2', country_id: 'country-vn', code: 'DN2', name: 'Đồng Nai', name_en: 'Dong Nai' },
    { id: 'province-brvt', country_id: 'country-vn', code: 'BRVT', name: 'Bà Rịa - Vũng Tàu', name_en: 'Ba Ria - Vung Tau' }
  ];
  ...
  for (const province of provinces) {
    await prisma.md_provinces.upsert({
      ...
      update: {},
      create: { ...province, updated_at: new Date() }
    });
  }
  console.log(`✅ md_provinces: seeded ${provinces.length} records`);

  // STEP 3: Seed cities (requires province_id)
  console.log('🏙️ Seeding md_cities...');
  const cities = [
    { id: 'city-hcm', province_id: 'province-hcm', code: 'HCM', name: 'TP. Hồ Chí Minh', name_en: 'Ho Chi Minh City' },
    { id: 'city-hn', province_id: 'province-hn', code: 'HN', name: 'Hà Nội', name_en: 'Hanoi' },
    { id: 'city-dn', province_id: 'province-dn', code: 'DN', name: 'Đà Nẵng', name_en: 'Da Nang' },
    { id: 'city-hp', province_id: 'province-hp', code: 'HP', name: 'Hải Phòng', name_en: 'Hai Phong' },
    { id: 'city-ct', province_id: 'province-ct', code: 'CT', name: 'Cần Thơ', name_en: 'Can Tho' },
    { id: 'city-thu-dau-mot', province_id: 'province-bd', code: 'TDM', name: 'Thủ Dầu Một', name_en: 'Thu Dau Mot' },
    { id: 'city-bien-hoa', province_id: 'province-dn2', code: 'BH', name: 'Biên Hòa', name_en: 'Bien Hoa' },
    { id: 'city-vung-tau', province_id: 'province-brvt', code: 'VT', name: 'Vũng Tàu', name_en: 'Vung Tau' }
  ];
  
  for (const city of cities) {
    await prisma.md_cities.upsert({
      where: { id: city.id },
      update: {},
      create: { ...city, updated_at: new Date() }
    });
  }
  console.log(`✅ md_cities: seeded ${cities.length} records`);

  // List of master data tables and their seed data (ALL REMAINING 42 TABLES)
  ...
  const masterDataTables = {
    ...
    // 49. md_iso_container_codes
    md_iso_container_codes: [
      { iso_code: '22G1', description: '20ft General Purpose' },
      { iso_code: '42G1', description: '40ft General Purpose' },
      { iso_code: '45G1', description: '45ft High Cube' },
      { iso_code: '22R1', description: '20ft Refrigerated' },
      { iso_code: '42R1', description: '40ft Refrigerated' },
      { iso_code: '22U1', description: '20ft Open Top' },
      { iso_code: '42U1', description: '40ft Open Top' },
      { iso_code: '22P1', description: '20ft Flat Rack' },
      { iso_code: '42P1', description: '40ft Flat Rack' },
      { iso_code: '22T1', description: '20ft Tank' }
    ],

    // 50. md_service_codes
    md_service_codes: [
      { code: 'STORAGE', name: 'Storage', description: 'Container storage service', billable: true },
      { code: 'TRANSPORT', name: 'Transport', description: 'Container transport service', billable: true },
      { code: 'INSPECTION', name: 'Inspection', description: 'Container inspection service', billable: true },
      { code: 'REPAIR', name: 'Repair', description: 'Container repair service', billable: true },
      { code: 'CLEANING', name: 'Cleaning', description: 'Container cleaning service', billable: true },
      { code: 'CERTIFICATION', name: 'Certification', description: 'Container certification service', billable: true },
      { code: 'FUMIGATION', name: 'Fumigation', description: 'Container fumigation service', billable: true },
      { code: 'SURVEY', name: 'Survey', description: 'Container survey service', billable: true }
    ],

    // 51. md_payment_method_types
    md_payment_method_types: [
      ...
      { code: 'CRYPTOCURRENCY', name: 'Cryptocurrency', description: 'Digital currency payment' }
    ],

    // 52. md_incoterms
    md_incoterms: [
      { code: 'EXW', name: 'Ex Works', description: 'Ex Works (named place of delivery)' },
      { code: 'FCA', name: 'Free Carrier', description: 'Free Carrier (named place of delivery)' },
      { code: 'CPT', name: 'Carriage Paid To', description: 'Carriage Paid To (named place of destination)' },
      { code: 'CIP', name: 'Carriage and Insurance Paid', description: 'Carriage and Insurance Paid To (named place of destination)' },
      { code: 'DAP', name: 'Delivered at Place', description: 'Delivered at Place (named place of destination)' },
      { code: 'DPU', name: 'Delivered at Place Unloaded', description: 'Delivered at Place Unloaded (named place of destination)' },
      { code: 'DDP', name: 'Delivered Duty Paid', description: 'Delivered Duty Paid (named place of destination)' },
      { code: 'FAS', name: 'Free Alongside Ship', description: 'Free Alongside Ship (named port of shipment)' },
      { code: 'FOB', name: 'Free on Board', description: 'Free on Board (named port of shipment)' },
      { code: 'CFR', name: 'Cost and Freight', description: 'Cost and Freight (named port of destination)' },
      { code: 'CIF', name: 'Cost, Insurance and Freight', description: 'Cost, Insurance and Freight (named port of destination)' }
    ],

    // 53. md_partner_types
    md_partner_types: [
      { code: 'SUPPLIER', name: 'Supplier', description: 'Container supplier partner' },
      { code: 'BUYER', name: 'Buyer', description: 'Container buyer partner' },
      { code: 'LOGISTICS', name: 'Logistics Provider', description: 'Logistics service provider' },
      { code: 'DEPOT', name: 'Depot Operator', description: 'Container depot operator' },
      { code: 'INSPECTOR', name: 'Inspector', description: 'Container inspection service provider' },
      { code: 'FINANCIER', name: 'Financier', description: 'Financial service provider' },
      { code: 'INSURANCE', name: 'Insurance Provider', description: 'Insurance service provider' },
      { code: 'SHIPPER', name: 'Shipper', description: 'Shipping line partner' }
    ],

    // 54. md_delivery_event_types (Added - was missing)
    md_delivery_event_types: [
      { code: 'CREATED', name: 'Tạo đơn giao hàng', description: 'Đơn giao hàng được tạo' },
      { code: 'SCHEDULED', name: 'Lên lịch giao hàng', description: 'Đã lên lịch giao hàng' },
      { code: 'PICKED_UP', name: 'Nhận hàng', description: 'Đã nhận hàng từ kho' },
  ...
  for (const [tableName, records] of Object.entries(masterDataTables)) {
    try {
      ...
      if (count === 0) {
        // Handle special cases for tables with different structures
        let dataWithIds;
        
        if (tableName === 'md_iso_container_codes') {
          // md_iso_container_codes uses iso_code instead of code
          dataWithIds = records.map(record => ({
            id: uuid(),
            ...record,
            updated_at: new Date()
          }));
        } else if (tableName === 'md_service_codes') {
          // md_service_codes has billable field
          dataWithIds = records.map(record => ({
            id: uuid(),
            ...record,
            updated_at: new Date()
          }));
        } else if (tableName === 'md_provinces') {
          // md_provinces has country_id relationship
          dataWithIds = records.map(record => ({
            id: uuid(),
            ...record,
            updated_at: new Date()
          }));
        } else if (tableName === 'md_cities') {
          // md_cities has province_code, country_code relationships
          dataWithIds = records.map(record => ({
            id: uuid(),
            ...record,
            updated_at: new Date()
...
async function main() {
  ...
  try {
    ...
    console.log('  ✓ Master Data Tables: 54 (ALL md_* tables)');
    console.log('');
