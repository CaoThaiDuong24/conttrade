      ...
      if (dealCount === 0) {
        await prisma.md_deal_types.createMany({
          data: [
            ...
            { id: uuid(), code: 'LEASE', name: 'Lease', description: 'Container lease transaction' },
            { id: uuid(), code: 'RENTAL', name: 'Rental', description: 'Short-term container rental' },
            { id: uuid(), code: 'SWAP', name: 'Swap', description: 'Container swap transaction' }
          ].map(item => ({ ...item, updated_at: new Date() })),
          skipDuplicates: true
        });
