...
export class ApiClient {
  ...
  private async executeRequest<TResponse = unknown, TBody = unknown>(options: ApiRequestOptions<TBody>): Promise<TResponse> {
    ...
    if (!effectiveBaseUrl || effectiveBaseUrl.trim() === '') {
      if (typeof window !== 'undefined') {
        // Client-side: ALWAYS use API_BASE_URL from env, NOT window.location.origin
        // This ensures API calls go to backend port 3006, not frontend port 3000
        effectiveBaseUrl = API_BASE_URL || 'http://localhost:3006';
        console.log('[API Client] Client-side: using API_BASE_URL from env:', effectiveBaseUrl);
      } else {
        // Server-side: use API_BASE_URL or fallback
        effectiveBaseUrl = API_BASE_URL || 'http://localhost:3006';
        console.log('[API Client] Server-side: using API_BASE_URL:', effectiveBaseUrl);
      }
    }
    ...
    try {
      ...
      if (path.startsWith('http://') || path.startsWith('https://')) {
        ...
        try {
          ...
          // If still fails, use API_BASE_URL fallback
          const fallbackBase = API_BASE_URL || 'http://localhost:3006';
