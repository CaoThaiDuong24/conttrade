...
export default function CreateOrderPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    deliveryDate: '',
    notes: '',
    rentalDurationMonths: 1 // ✅ NEW: Default rental duration for RENTAL type
  });
  ...
  useEffect(() => {
    // ✅ FIX: Set default rental duration when listing is loaded
    if (listing && listing.dealType === 'RENTAL') {
      setFormData(prev => ({
        ...prev,
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    } else if (field === 'rentalDurationMonths') {
      // ✅ FIX: Handle rental duration as number
      setFormData(prev => ({
        ...prev,
  ...
  const validateForm = () => {
    ...
    // ✅ FIX: Validate rental duration for RENTAL type
    if (listing?.dealType === 'RENTAL') {
      if (!formData.rentalDurationMonths || formData.rentalDurationMonths <= 0) {
        setError('Vui lòng nhập thời gian thuê hợp lệ');
        return false;
      }
      if (listing.min_rental_duration && formData.rentalDurationMonths < listing.min_rental_duration) {
        setError(`Thời gian thuê tối thiểu là ${listing.min_rental_duration} tháng`);
        return false;
      }
      if (listing.max_rental_duration && formData.rentalDurationMonths > listing.max_rental_duration) {
        setError(`Thời gian thuê tối đa là ${listing.max_rental_duration} tháng`);
        return false;
      }
    }
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/orders/from-listing`, {
        ...
        body: JSON.stringify({
          ...
          quantity: listing?.dealType === 'SALE' ? quantity : undefined,
          deliveryAddress: formData.deliveryAddress,
          notes: formData.notes || undefined,
          deal_type: listing?.dealType, // ✅ FIX: Send deal_type
          rental_duration_months: listing?.dealType === 'RENTAL' ? formData.rentalDurationMonths : undefined // ✅ FIX: Send rental duration
        }),
      });
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {listing?.dealType === 'SALE' && listing?.totalQuantity && listing.totalQuantity > 1 && (
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Tổng giá: {new Intl.NumberFormat('vi-VN').format(parseFloat(formData.agreedPrice || '0') * quantity)} {listing.priceCurrency}
                    </p>
                  </div>
                )}

                {/* Rental Duration for RENTAL type */}
                {listing?.dealType === 'RENTAL' && (
                  <div className="space-y-2">
                    <Label htmlFor="rentalDurationMonths">Thời gian thuê (tháng) *</Label>
                    <div className="flex items-center gap-3">
                      <Input
                        id="rentalDurationMonths"
                        type="number"
                        min={listing.min_rental_duration || 1}
                        max={listing.max_rental_duration || 60}
                        value={formData.rentalDurationMonths}
                        onChange={(e) => handleInputChange('rentalDurationMonths', e.target.value)}
                        className="w-32"
                        required
                      />
                      <div className="text-sm text-muted-foreground">
                        {listing.min_rental_duration && listing.max_rental_duration && (
                          <span>
                            (Tối thiểu: {listing.min_rental_duration}, Tối đa: {listing.max_rental_duration})
                          </span>
                        )}
                      </div>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      Tổng giá cho {formData.rentalDurationMonths} tháng: {new Intl.NumberFormat('vi-VN').format(parseFloat(formData.agreedPrice || '0') * (formData.rentalDurationMonths || 1))} {listing.priceCurrency}
                    </p>
                  </div>
                )}
