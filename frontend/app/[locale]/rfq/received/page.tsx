...
export default function RFQReceivedPage() {
  ...
  useEffect(() => {
    ...
    if (!hasFetched) {
      console.log('🎬 useEffect triggered - calling fetchRFQs');
      setHasFetched(true);
      fetchRFQs();
    } else {
      console.log('⏭️ useEffect triggered but already fetched - skipping');
    }
  ...
  const fetchRFQs = async () => {
    ...
    console.log(`\n${'='.repeat(80)}`);
    console.log(`🔄 [${requestId}] START fetchRFQs at ${timestamp}`);
    ...
    try {
      ...
      console.log(`✅ [${requestId}] Token found:`, token.substring(0, 30) + '...');
      ...
      try {
        ...
        console.log(`🔓 [${requestId}] Token decoded:`, decoded);
        console.log(`👤 [${requestId}] User ID in token:`, decoded.userId);
        console.log(`📧 [${requestId}] Email in token:`, decoded.email);
      } catch (e) {
        console.error(`❌ [${requestId}] Cannot decode token:`, e);
      }

      // Fallback for API URL if env not set
      const apiUrl = '/api/v1';
      const fullUrl = `${apiUrl}/rfqs?view=received&_t=${Date.now()}`; // Add timestamp to prevent cache

      console.log(`📡 [${requestId}] Calling API:`, fullUrl);
      ...
      if (response.ok) {
        ...
        console.log(`=== [${requestId}] RFQ API RESPONSE DETAILED DEBUG ===`);
        console.log('Response structure:', data);
        console.log('data.success:', data.success);
        console.log('data.data type:', Array.isArray(data.data) ? 'Array' : typeof data.data);
        console.log('RFQ Count:', data.data?.length || 0);
        console.log('First 3 RFQs raw data:', data.data?.slice(0, 3));
        console.log('==========================================');
        ...
        console.log(`=== [${requestId}] MAPPED RFQs DETAILS ===`);
        console.log('Total mapped:', mappedRfqs.length);
        console.log('First RFQ mapped:', mappedRfqs[0]);
        console.log('Buyer names:', mappedRfqs.map((r: RFQ) => r.buyerName));
        console.log('===========================');
        
        console.log(`✅ [${requestId}] Setting ${mappedRfqs.length} RFQs to state`);
        setRfqs(mappedRfqs);
      } else {
        const errorData = await response.text();
        console.error(`❌ [${requestId}] Failed to fetch RFQs:`, response.status);
        console.error(`❌ [${requestId}] Error response:`, errorData);
      }
    } catch (error) {
      console.error(`❌ [${requestId}] Error fetching RFQs:`, error);
    } finally {
      console.log(`🏁 [${requestId}] END fetchRFQs`);
      setIsLoading(false);
    }
  };
