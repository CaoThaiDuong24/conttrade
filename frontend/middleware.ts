...
const ROUTE_PERMISSIONS = {
  ...
  // Buyer routes - Shopping Cart
  "/cart": "PM-001", // Basic authenticated access - requires login
  "/cart/checkout": "PM-040", // CREATE_ORDER - requires order creation permission
...
export default async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // Default aligns with backend/src/server.ts to avoid signature mismatch in local dev
    ...
    if (cached && now - cached.timestamp < CACHE_TTL) {
      ...
      try {
        ...
        const backendUrl =
          process.env.API_URL_INTERNAL || "http://lta-backend:3006";
        ...
        if (meResponse.ok) {
          ...
          if (meData.success && meData.data?.user) {
            ...
            console.log(
              "✅ Got REALTIME permissions from database via /auth/me (cached for 30s)"
            );
        ...
        console.warn(
          "⚠️ Failed to fetch realtime permissions, using JWT fallback:",
          error
        );
    ...
    const hasRequiredPermission = requiredPermission
      ? hasPermission(userPermissions, userRoles, requiredPermission)
      : true;
    ...
    if (requiredPermission && !hasRequiredPermission) {
      console.log(
        ...
        requiredPermission,
        '- But allowing access to show "Under Development" page'
      );
    }

    // Add user info to headers for server components
    const response =
      intlResponse instanceof Response ? intlResponse : NextResponse.next();
...
function getRequiredPermission(routePath: string) {
  ...
  if (routePath.startsWith("/admin")) {
    console.log(
      "→ Defaulting to PM-072 (ADMIN_VIEW_DASHBOARD) for admin route"
    );
    return "PM-072";
  }
  ...
  if (routePath.startsWith("/account")) {
    console.log(
      "→ Defaulting to PM-001 (basic authenticated access) for account route"
    );
    return "PM-001";
  }
  ...
  console.log(
    "→ Defaulting to PM-001 (basic authenticated access) for undefined route"
  );
  return "PM-001";
}
...
async function getUserRoles(
  ...
  if (email) {
    ...
    if (
      email.includes("depot@") ||
      email.includes("depot-staff@") ||
      email.includes("staff@depot")
    ) {
    ...
    // Config Manager role
    if (
      email.includes("config@") ||
      email.includes("config-manager@") ||
      email.includes("configuration@")
    ) {
    ...
    // Price Manager role
    if (
      email.includes("price@") ||
      email.includes("price-manager@") ||
    ...
    if (
      ...
      email.includes("cs@") ||
      email.includes("customer-support@")
    ) {
...
async function getUserPermissions(roles: string[]): Promise<string[]> {
  ...
  if (roles.includes("admin")) {
    return [
      "PM-001",
      "PM-002",
      "PM-003", // View, search listings, view seller profile
      "PM-010",
      "PM-011",
      "PM-012",
      "PM-013",
      "PM-014", // All listing permissions
      "PM-020",
      "PM-021",
      "PM-022",
      "PM-023",
      "PM-024", // RFQ permissions
      "PM-030",
      "PM-031", // Inspection permissions
      "PM-040",
      "PM-041",
      "PM-042",
      "PM-043", // Order permissions
      "PM-050", // Reviews
      "PM-060",
      "PM-061", // Dispute permissions
      "PM-070",
      "PM-071",
      "PM-072",
      "PM-073",
      "PM-074", // Admin permissions
      "PM-080",
      "PM-081",
      "PM-082",
      "PM-083",
      "PM-084",
      "PM-085",
      "PM-086", // Depot permissions
      "PM-090",
      "PM-091", // Finance permissions
      "PM-100",
      "PM-101",
      "PM-102",
      "PM-103",
      "PM-104",
      "PM-105", // Rental contract management
      "PM-110",
      "PM-111",
      "PM-112", // Rental container management
      "PM-120",
      "PM-121",
      "PM-122",
      "PM-123", // Maintenance management
      "PM-130",
      "PM-131",
      "PM-132",
      "PM-133", // Buyer rental permissions
      "PM-140",
      "PM-141", // Rental finance & reports
      "PM-113",
      "PM-114",
      "PM-115",
      "PM-116",
  ...
  if (roles.includes("depot_manager")) {
    return [
      "PM-001",
      "PM-002", // View listings
      "PM-030",
      "PM-031", // Inspection permissions
      "PM-040",
      "PM-042",
      "PM-043", // Order and delivery
      "PM-050", // Reviews
      "PM-080",
      "PM-081",
      "PM-082",
  ...
  if (roles.includes("price_manager")) {
    return [
      "PM-001",
      "PM-002", // View and search listings
      "PM-072",
      "PM-073",
      "PM-074", // Admin dashboard and pricing
    ];
  }
  ...
  if (roles.includes("seller")) {
    return [
      ...
      "PM-012",
      "PM-013",
      "PM-014", // Seller listing permissions
      "PM-020",
      "PM-021",
      "PM-022",
      "PM-023", // RFQ, quotes, and Q&A management
      "PM-040",
      "PM-042", // Orders and delivery
      "PM-050", // Reviews
      "PM-060", // Disputes
      "PM-090", // Billing
      "PM-100",
  ...
  return [
    "PM-001",
    "PM-002",
    "PM-003", // View and search listings
    "PM-020",
    "PM-022", // RFQ
    ...
    "PM-041",
    "PM-042",
    "PM-043", // Orders, payment, delivery, receipt
    "PM-050", // Reviews
    "PM-060", // Disputes
    "PM-130",
    "PM-131",
...
function hasPermission(
  ...
  if (requiredPermission === "PM-001" && userLevel >= ROLE_HIERARCHY.buyer) {
    console.log(
      "✅ Allowing PM-001 (VIEW_PUBLIC_LISTINGS) for authenticated user"
    );
    return true;
  }
