...
interface Notification {
  id: string;
  type: 'payment_received' | 'order_completed' | 'order_created' | 'rfq_received' | 'quote_received' | 'quote_accepted' | 'quote_rejected' | 'container_ready' | 'system' | 'reminder' | 'review_response' | 'delivery_reviewed';
  title: string;
  message: string;
  data: any;
  read: boolean;
...
export function NotificationBell() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          // Check for new review_response notifications
          const newReviewResponses = sortedNotifications.filter((n: Notification) => 
            n.type === 'review_response' && 
            !n.read && 
            !previousNotificationIds.current.has(n.id)
          );
          
          // Show toast for new review responses
          if (newReviewResponses.length > 0 && previousNotificationIds.current.size > 0) {
            newReviewResponses.forEach((notif: Notification) => {
              showInfo(`💬 ${notif.title}: ${notif.message}`, 5000);
            });
          }
          
          // Update previous notification IDs
          previousNotificationIds.current = new Set(sortedNotifications.map((n: Notification) => n.id));
  ...
  const getNotificationIcon = (notification: Notification) => {
    ...
    switch (notification.type) {
      ...
      case 'review_response':
        return <MessageSquare className={cn(baseClasses, "text-pink-600")} />;
      case 'delivery_reviewed':
        return <CheckCircle className={cn(baseClasses, "text-yellow-600")} />;
      case 'system':
  ...
  useEffect(() => {
    ...
    // Poll every 15 seconds for new notifications (reduced from 30s)
