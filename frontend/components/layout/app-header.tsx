...
export function AppHeader({ 
  ...
  // Debug log để kiểm tra userInfo
  console.log('🎯 AppHeader Debug:', {
    isAuthenticated,
    userInfo,
    roles: userInfo?.roles,
    isBuyer: userInfo?.roles?.includes('buyer'),
    isSeller: userInfo?.roles?.includes('seller')
  });
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            {isAuthenticated && userInfo?.roles && userInfo.roles.length > 0 && (() => {
              ...
              console.log('🔍 Button Check:', { roles, isBuyer, isSeller, shouldShow: isBuyer && !isSeller });
              
              // Chỉ hiển thị nếu là buyer và KHÔNG phải seller
              if (isBuyer && !isSeller) {
                return (
                  <Button 
                    asChild 
                    variant="default"
                    className="hidden sm:flex items-center gap-2 bg-gradient-to-r from-primary to-primary/90 hover:from-primary/90 hover:to-primary shadow-lg hover:shadow-xl transition-all"
                  >
                    <Link href="/vi/become-seller">
                      <Building className="h-4 w-4" />
                      <span className="font-medium">Trở thành Nhà cung cấp</span>
                    </Link>
                  </Button>
                );
              }
                  ...
                  {isAuthenticated && userInfo?.roles && userInfo.roles.length > 0 && (() => {
                    ...
                    if (isBuyer && !isSeller) {
                      return (
                        ...
                        >
                          <Link href="/vi/become-seller" className="flex items-center justify-center gap-2" onClick={() => setIsMenuOpen(false)}>
                            <Building className="h-4 w-4" />
                            <span className="font-medium">Trở thành Nhà cung cấp</span>
                          </Link>
                        </Button>
