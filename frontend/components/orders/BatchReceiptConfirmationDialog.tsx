...
export default function BatchReceiptConfirmationDialog({
  deliveryId,
  batchNumber,
  totalBatches,
  containers,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If disputes were created, notify user
      if (result.data?.disputes_created && result.data.disputes_created.length > 0) {
        message += `\n\nĐã tự động tạo ${result.data.disputes_created.length} tranh chấp cho container hư hỏng nặng.`;
      }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Lô {batchNumber}/{totalBatches} - {containers.length} container{containers.length > 1 ? 's' : ''}: {containers.map(c => c.container_iso_code).join(', ')}
          </DialogDescription>
        </DialogHeader>
